    ...
    public class PROTOCOL_BATTLE_TIMERSYNC_REQ : ReceivePacket
    {
        ...
        public override void run()
        {
            try
            {
                ...
                if ((TimeRemaining == 0 || TimeRemaining > 0x80000000) && !room.swapRound && CompareRounds(room, Round) && room._state == RoomState.Battle)
                {
        ...
        private void EndRound(Room room, bool isBotMode)
        {
            ...
            catch (Exception ex)
            {
                if (room != null)
                {
                    Logger.error("PROTOCOL_BATTLE_TIMERSYNC_REQ: RoomId: " + room._roomId + " ChannelId: " + room._channelId + " RoomType: " + room.room_type);
                }
