                ...
                if (p != null && p.player_name.Length > 0 && p._room == null && p._match == null && p.getChannel(out ch))
                {
                    lock (ch._rooms)
                    {
                        for (int ridx = 0; ridx < ch._rooms.Count; ridx++)
                        {
                            ...
                            if (QuickSelect.MapId == (int)room.mapId && QuickSelect.Rule == room.rule && QuickSelect.StageOptions == room.stage && QuickSelect.Type == (int)room.room_type && room.password.Length == 0 && room.Limit == 0 && (!room.kickedPlayers.Contains(p.player_id) || p.HaveGMLevel()))
                            {
