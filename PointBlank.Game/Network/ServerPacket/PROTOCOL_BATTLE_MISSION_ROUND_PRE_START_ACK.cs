...
namespace PointBlank.Game.Network.ServerPacket
{
    public class PROTOCOL_BATTLE_MISSION_ROUND_PRE_START_ACK : SendPacket
    {
        ...
        public override void write()
        {
            ...
            else if (_r.room_type == RoomType.Boss || _r.room_type == RoomType.CrossCounter)
            {
                int TRex = _dinos.Count == 1 || _r.room_type == RoomType.CrossCounter ? 255 : _r.TRex;
                writeC((byte)TRex);
                writeC(10);
                for (int i = 0; i < _dinos.Count; i++)
                {
                    int slotId = _dinos[i];
                    if (slotId != _r.TRex && _r.room_type == RoomType.Boss || _r.room_type == RoomType.CrossCounter)
                    {
                        writeC((byte)slotId);
                    }
                }
