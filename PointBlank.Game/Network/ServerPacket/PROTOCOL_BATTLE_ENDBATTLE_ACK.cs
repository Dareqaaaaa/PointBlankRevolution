...
namespace PointBlank.Game.Network.ServerPacket
{
    public class PROTOCOL_BATTLE_ENDBATTLE_ACK : SendPacket
    {
        ...
        public PROTOCOL_BATTLE_ENDBATTLE_ACK(Account p)
        {
            ...
            {
                r = p._room;
                if (r.room_type == RoomType.Tutorial)
                {
                    Winner = 0;
                }
                else
        ...
        public override void write()
        {
            ...
            {
                writeH((ushort)(r.room_type == RoomType.Boss ? r.red_dino : (r.room_type == RoomType.CrossCounter ? r._redKills : r.red_rounds)));
                writeH((ushort)(r.room_type == RoomType.Boss ? r.blue_dino : (r.room_type == RoomType.CrossCounter ? r._blueKills : r.blue_rounds)));
                for (int i = 0; i < 16; i++)
                {
